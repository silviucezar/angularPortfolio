...
let FooterComponent = class FooterComponent {
    constructor(pageLogic, http, initService) {
        ...
        this.http.activeRequestsCount$.subscribe((requestDetails) => {
...
/* harmony import */ var _url_listener_service__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./url-listener.service */ "./src/app/Services/url-listener.service.ts");
...
let InitService = class InitService {
    constructor(_document, lazyService, pageLogic, urlListener) {
    ...
    setScrollEvent() {
        ...
        ;
        this.urlListener.urlSubscriptionBehaviorSubject$.subscribe((urlSubscription) => {
            if (urlSubscription.dataToFetch.match(/skills|jobs/))
                return;
            this.urlSubscription = urlSubscription;
            setTimeout(() => {
                globalContentContainer.scrollTo({
                    top: urlSubscription.dataToFetch === 'about_me' ? 0 : this.viewport.contentComponentsPosition[urlSubscription.dataToFetch].component.offsetTop - 200,
                    behavior: "smooth"
                });
                setTimeout(() => {
        ...
        if (window.ontouchmove === null) {
            ...
            window.ontouchend = (event) => triggerContentScroll(event);
        }
